...
export default class TabulaConiuncta<Hoc extends Multiplex> extends Tabula<Hoc> {
  ...
  async plenetur(): Promise<void> {
    ...
    const areSetsEqual: (prima: Set<any>, secunda: Set<any>) => boolean = (await import('@neoncitylights/sets')).areSetsEqual;

    const tabulaPrima: Hoc[] = await prima.tabulentur();
    const tabulaSecunda: Hoc[] = await secunda.tabulentur();

    if (areSetsEqual(new Set(tabulaPrima.map(valedictor)), new Set(tabulaSecunda.map(valedictor)))) {
